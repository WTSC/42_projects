...
{
	if (to_deg(w3d->ray) != 180 && to_deg(w3d->ray) != 0)
	{
		if (to_deg(w3d->ray) < 180)
...
unsigned int		w3d_cast_horizontal(t_w3d *w3d)
{
	...
	{
		printf("TAFOTE\n");
		printf("hpoint foint at coordinates (%d %d)\n", grid.x, grid.y);
	...
	else
	{
		...
		printf("horizontal point foint at coordinates (%d %d)\n", grid.x, grid.y);
		return ((unsigned int)(sqrt(pow(w3d->pos.x - pt.x, 2) + pow(w3d->pos.y - pt.y, 2))));
}
